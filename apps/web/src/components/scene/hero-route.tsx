...
export function HeroRoute({ note }: { note: string }) {
  return <>
    <svg className="hero-route" viewBox="0 0 1672 941" preserveAspectRatio="none" aria-hidden="true">
      ...
      <g className="route-glow"><path d={MAIN} pathLength={1} className="route-main"/>{BRANCHES.map((b, i) => <path key={i} d={b.d} pathLength={1} className="route-branch" style={{ "--at": b.at } as React.CSSProperties}/>)}</g>
      <g className="route-ink"><path d={MAIN} pathLength={1} className="route-main" data-route-path="main"/>{BRANCHES.map((b, i) => <path key={i} d={b.d} pathLength={1} className="route-branch" style={{ "--at": b.at } as React.CSSProperties}/>)}</g>
