...
export default function CaseOverview() {
  const { org } = useOrg(); const { case: id } = useParams<{ case: string }>();
  ...
  return <main id="main-content" className="page-shell">
    ...
    <div className="case-grid">
      <section className="surface stack" aria-labelledby="area-heading"><h2 id="area-heading">Investigation area</h2>
        {c.current_assessment_id ? <p>An assessment is recorded for this case.</p> : <p><strong>Investigation area not yet established.</strong> {c.network_id ? "A local network is recorded; readiness checks decide whether localization can run." : "Map verification needed: no reviewed local stream network is recorded for this case yet."}</p>}
      </section>
