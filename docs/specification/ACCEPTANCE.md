# Upstream — release acceptance contract
All mandatory checks are evaluated against working software, not screenshots or promises.

## How to report results

Create `docs/release-results.md` listing each ID below, command/manual procedure, result, date and artifact. Use PASS, FAIL, or BLOCKED_EXTERNAL. Do not mark an unrun check PASS. External provider checks may be BLOCKED_EXTERNAL only where the core app has the specified honest fallback; local/auth/storage/scientific checks cannot be deferred that way.

Tests use at least two organizations, two contributors, a monitor, a coordinator, an expert, an admin without expert qualification, a revoked member and a recipient. Example data is isolated.

## A. Setup and durable application
...
## B. Reports, participation and unmapped streams
...
## D. Tasks, instruments and measurements
...
## F. Review, dependency propagation and delivery
...
## I. Visual, motion and accessibility
