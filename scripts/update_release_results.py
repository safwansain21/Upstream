...
CMD = ('`.venv/Scripts/python.exe -m pytest tests -q -p no:cacheprovider -rA` '
       '(local Supabase + `pnpm seed:example`; API :8000, worker, `next start` :3000) - 96 passed, 2026-09-21')
...
PASSES = {
    ...
    'D01': [F + 'test_assignment_checks_qualification_instrument_version_and_limitations'],
    ...
    'D08': [F + 'test_replicates_are_individual_and_idempotent', P + 'test_raw_shared_scopes_and_independent_compensation'],
    ...
    'D14': [F + 'test_assignment_checks_qualification_instrument_version_and_limitations'],
...
PARTIAL = {
    ...
    'D09': 'Not tested: calibration id is stored at capture; no expiry-after-reading test yet.',
